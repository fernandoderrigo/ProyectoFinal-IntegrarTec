...
export default function Gender({ showFullGender }) {
  ...
  const SkeletonGender = () => (
    <article className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 px-4 py-5 animate-pulse">
      {[...Array(4)].map((_, index) => (
        <div
          key={index}
  ...
  return (
    <motion.article
      className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 px-4 py-5"
      variants={container}
      initial="hidden"
      animate="show"
    >
